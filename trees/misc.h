...
inline void * SafeMalloc(size_t size) {
  ...
  if ( (result = malloc(size)) ) { /* assignment intentional */
    ...
    sprintf(errorMessagePartTwo, "Calling sbrk(0) gives %ld\n",(intptr_t)sbrk(0));
...
inline void * SafeCalloc(int numberOfElements , size_t size) {
  ...
  if ( (result = calloc(numberOfElements, size)) )
    ...
    printf("memory overflow: calloc failed in SafeCalloc(%i,%lu).", numberOfElements, (unsigned long)size);
//    printf("memory overflow: calloc failed in SafeCalloc(%i,%zu).", numberOfElements, size);
//    printf("sbrk(0) gives %x\n",(int)sbrk(0));
    printf("sbrk(0) gives %ld\n",(intptr_t)sbrk(0));
