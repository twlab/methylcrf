...
IntervalTree::IntervalTree()
{
  ...
  nil->red = 0;
  nil->key = nil->high = nil->maxHigh = MIN_ULONG;
  nil->storedInterval = NULL;
 
  root = new IntervalTreeNode;
  root->parent = root->left = root->right = nil;
  root->key = root->high = root->maxHigh = MAX_ULONG;
  root->red=0;
...
void IntervalTree::LeftRotate(IntervalTreeNode* x) {
  ...
  Assert((nil->maxHigh=MIN_ULONG),
         "nil->maxHigh != MIN_ULONG in ITLeftRotate");
#endif
}
...
void IntervalTree::RightRotate(IntervalTreeNode* y) {
  ...
  Assert((nil->maxHigh=MIN_ULONG),
	 "nil->maxHigh != MIN_ULONG in ITRightRotate");
#endif
}
...
void IntervalTree::TreeInsertHelp(IntervalTreeNode* z) {
  ...
  Assert((nil->maxHigh=MIN_ULONG),
	 "nil->maxHigh != MIN_ULONG in ITTreeInsertHelp");
#endif
}
...
IntervalTreeNode * IntervalTree::Insert(Interval * newInterval)
{
  ...
  Assert((nil->maxHigh=MIN_ULONG),
	 "nil->maxHigh != MIN_ULONG in ITTreeInsert");
#endif
}
...
void IntervalTree::DeleteFixUp(IntervalTreeNode* x) {
  ...
  Assert((nil->maxHigh=MIN_ULONG),
	 "nil->maxHigh != MIN_ULONG in ITDeleteFixUp");
#endif
}
...
Interval * IntervalTree::DeleteNode(IntervalTreeNode * z){
  ...
  if (y != z) { /* y should not be nil in this case */
    ...
    Assert((nil->maxHigh=MIN_ULONG),"nil->maxHigh != MIN_ULONG in ITDelete");
#endif
  } else {
    FixUpMaxHigh(x->parent);
...
int Overlap(unsigned long a1, unsigned long a2, unsigned long b1, unsigned long b2) {
  if (a1 <= b1) {
    return( (b1 <= a2) );
  } else {
    return( (a1 <= b2) );
...
TemplateStack<void *> * IntervalTree::Enumerate(unsigned long low, 
							unsigned long high)  {
...
int IntervalTree::CheckMaxHighFieldsHelper(IntervalTreeNode * y, 
				    const unsigned long currentHigh,
				    int match) const
{
  if (y != nil) {
...
void IntervalTree::CheckAssumptions() const {
 VERIFY(nil->key == MIN_ULONG);
 VERIFY(nil->high == MIN_ULONG);
 VERIFY(nil->maxHigh == MIN_ULONG);
 VERIFY(root->key == MAX_ULONG);
 VERIFY(root->high == MAX_ULONG);
 VERIFY(root->maxHigh == MAX_ULONG);
