...
{
  if (argc != 2) {
    Tcl_SetResult(interp, "Wrong # arguments.\nUsage: intTreeCreate nameOfTree\n", TCL_STATIC);
    return TCL_ERROR;
  } else if (NULL != global_IntervalTreesInTcl.Search(argv[1])) {
    Tcl_SetResult(interp, "An interval tree with that name already exists!\n", TCL_STATIC);
    return TCL_ERROR;
  } else {
...
int DeleteIntervalTreeInTcl(ClientData, Tcl_Interp * interp, int argc, char ** argv) {
...
int QueryIntervalTreeInTcl(ClientData, Tcl_Interp * interp, int argc, char ** argv) {
  int low;
  int high;
  IntervalTree * tree;
  if (argc != 4) {
			   ...
			   char ** argv)
{
  ...
  if (argc != 4) {
    ...
    Tcl_SetResult(interp, "An interval tree with that name doesn't exist!\n", TCL_STATIC);
    return TCL_ERROR;
  } else {
    if ( (TCL_OK != Tcl_GetInt(interp,argv[2],&low)) || (TCL_OK != Tcl_GetInt(interp,argv[3],&high)) )
      return TCL_ERROR;
    else {
